...
function PlatformBase (id, name, packageName, baseDir, extendedCfg) {
  ...
  */
  ...
  /**
  * Returns the output root for PWA or PolyFills
  */
  self.getOutputRoot = function(rootDir) {
    // update received rootDir
    if (rootDir && self.isPWA) {
      var regex = new RegExp(constants.POLYFILLS_FOLDER + '$');
      return rootDir.replace(regex, constants.PWA_FOLDER);
    }
  ...
  */
  self.getOutputFolder = function(rootDir) {
    ...
    return path.join(self.getOutputRoot(rootDir), targetFolder);
  }
