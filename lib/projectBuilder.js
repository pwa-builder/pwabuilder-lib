...
function createApps (w3cManifestInfo, rootDir, platforms, options, callback) {
  ...
  .then(function (platformModules) {
    ...
    return serviceWorkerTools.getRegisteredServiceWorkers(w3cManifestInfo.content.start_url).then(function(serviceWorkers) {
      if (serviceWorkers.length < 1) {
        log.warn('There is no service worker registering code at start URL page');
      }
    }).thenResolve(platformModules);
  })
  .then(function (platformModules) {
